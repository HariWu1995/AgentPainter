        ...
        if (episode_steps >= max_step) and max_step:
            if step > args.warmup:
                ...
                if episode > 0 and validate_interval > 0 and episode % validate_interval == 0:
                    ...
                    if debug: 
                        prRed('Step_{:07d}: mean_reward:{:.3f} mean_dist:{:.3f} var_dist:{:.3f}'.format(step - 1, np.mean(reward), np.mean(dist), np.var(dist)))
...
if __name__ == "__main__":
    ...
    agent = DDPG(args.batch_size, args.env_batch, args.max_step, \
                 args.tau, args.discount, args.rmsize, writer, args.resume, args.output)
